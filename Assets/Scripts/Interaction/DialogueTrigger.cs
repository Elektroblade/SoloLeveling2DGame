...
public class DialogueTrigger : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D col)
    {
        ...
        if (col.gameObject == NewPlayer.Instance.gameObject && !sleeping && !completed && NewPlayer.Instance.grounded)
        {
            ...
            if (autoHit || (Input.GetAxis("Submit") > 0))
            {
                ...
                if (requiredItem == "" && requiredCoins == 0 || GameManager.Instance.CheckForInventoryItem(requiredItem) == null && requiredCoins == 0 || (requiredCoins != 0 && NewPlayer.Instance.coins < requiredCoins))
                {
                ...
                else if (requiredCoins == 0 && GameManager.Instance.CheckForInventoryItem(requiredItem) != null || (requiredCoins != 0 && NewPlayer.Instance.coins >= requiredCoins))
                {
                    if (dialogueStringB != "")
                    {
    ...
    public void UseItem()
    {
        if (!completed)
        {
            ...
            if (GameManager.Instance.CheckForInventoryItem(requiredItem) != null)
            {
    ...
    public void Collect()
    {
        if (!completed)
        {
            if (getWhichItem != "")
            {
                GameManager.Instance.GiveItem(getWhichItem);
            }
